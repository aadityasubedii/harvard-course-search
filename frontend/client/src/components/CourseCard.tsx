...
export default function CourseCard({ course, onClick }: CourseCardProps) {
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Course Added',
        description: `${course.course_id} has been added to your cart.`,
      });
    } catch (error) {
  ...
  const handleSaveForLater = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Course Saved',
        description: `${course.course_id} has been saved for later.`,
      });
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row">
        <div className="p-4 flex-1">
          <div className="flex justify-between items-start gap-2">
            <h3 className="font-medium text-gray-900">{course.course_id}: {course.title}</h3>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <span className="text-xs bg-gray-100 px-2.5 py-1 rounded-md flex items-center">
              ...
              {formatClassSize(course.class_size)}
            </span>
            <span className="text-xs bg-gray-100 px-2.5 py-1 rounded-md flex items-center">
              <i className="ri-time-line mr-1.5 text-primary/70"></i>
              {course.average_workload || "Unknown"}
            </span>
            {course.course_id.includes('GENED') && (
              <span className="text-xs bg-blue-100 text-blue-600 px-2.5 py-1 rounded-md flex items-center">
          ...
          <p className="text-xs mt-3 text-gray-600 line-clamp-2">{course.course_description}</p>
        </div>
