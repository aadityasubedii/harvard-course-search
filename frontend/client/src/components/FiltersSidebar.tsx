  ...
  return (
    <div className={`md:flex flex-col w-72 bg-gray-50 p-5 border-r border-gray-200 ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin space-y-4">
        ...
        <div className="filter-section">
          <h3 className="filter-heading flex items-center">
            <i className="ri-calendar-line mr-2 text-primary/70"></i>
            Semester
          </h3>
          <Select
          ...
          >
            ...
            <SelectContent>
              <SelectGroup>
                ...
                <SelectItem value="fall" className="text-sm">Fall 2024</SelectItem>
                <SelectItem value="spring" className="text-sm">Spring 2025</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="filter-section">
          ...
          <div className="px-1">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-gray-600">Difficulty (workload + reviews)</span>
            ...
            <div className="flex items-center justify-between mb-1 mt-4">
              <span className="text-xs text-gray-600">Max Workload (hours)</span>
              <span className="text-xs font-medium text-primary">{filters.workloadHours ? `<${filters.workloadHours} hrs` : 'No limit'}</span>
            </div>
            <Slider
              defaultValue={[filters.workloadHours || 25]}
              max={20}
              min={2}
              onValueChange={(value) => {
                // Custom step logic to allow for 2, 5, 10, 15, 20, >20
                const steps = [2, 5, 10, 15, 20, 25];
                // Find closest step value
                const closest = steps.reduce((prev, curr) => 
                  Math.abs(curr - value[0]) < Math.abs(prev - value[0]) ? curr : prev
                );
                handleSliderChange('workloadHours', [closest]);
              }}
              className="my-2"
            />
