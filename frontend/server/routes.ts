...
`;
      ...
      // Maps for day codes and sample dates (April 2025)
      ...
      // Standard class times for fallback if schedule is missing
      const defaultStartTime = '100000'; // 10:00 AM
      const defaultEndTime = '113000';   // 11:30 AM
      
      // Process each course
      for (const course of cartCourses) {
        console.log(`Processing course for calendar: ${course.course_id}`);
        
        try {
          // Ensure we have a schedule (or use default)
          const schedule = course.schedule || 'Mon 10-11:30am';
          console.log(`Course schedule: ${schedule}`);
          
          // Parse the schedule
          const { days, startTime, endTime } = parseScheduleToIcalTimes(schedule);
          
          // For each day the class occurs, create an event
          for (const day of days) {
            const date = dates[day] || dates.MO;
            
            // Safe versions of fields to prevent undefined errors
            const courseId = course.course_id || 'COURSE-ID-MISSING';
            const courseTitle = course.title || 'Course Title';
            const courseProfessor = course.professor || 'TBA';
            const courseDescription = course.course_description || '';
            
            // Create a safe UID that won't have spaces or special characters
            const safeId = courseId.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
            
            // Add the event
            icalContent += `
BEGIN:VEVENT
SUMMARY:${courseId}: ${courseTitle}
UID:harvard-${safeId}-${course.id}-${day}
SEQUENCE:0
STATUS:CONFIRMED
TRANSP:OPAQUE
...
DESCRIPTION:Professor: ${courseProfessor}\\nCourse ID: ${courseId}\\n${courseDescription.substring(0, 200)}
END:VEVENT
`;
          }
        } catch (courseError) {
